/* 健康模块：饮水 / 步数 / 睡眠 / 运动 / 心情 + 一周趋势 */
(function () {
  const S = WB.store, U = WB.util;

  function getHealth() {
    return S.get('health', {
      goals: { steps: 8000, water: 8, sleep: 8, exercise: 30 },
      days: {}
    });
  }
  ...
  function today(h) {
    const k = U.todayKey();
    if (!h.days[k]) h.days[k] = { steps: 0, water: 0, sleep: 0, exercise: 0, mood: null };
    return h.days[k];
  }
  ...
  function wire(root, h) {
    ...
    root.querySelectorAll('#moodChips .chip').forEach(c => {
      c.onclick = () => { t.mood = Number(c.dataset.m); save(h); refresh(); };
    });
    ...
    ['#gWater', '#gSteps', '#gSleep', '#gEx'].forEach(s => {
